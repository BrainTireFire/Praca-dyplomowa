...
export default function ShortRestModalGM({
  membersList,
  onCloseModal,
}: {
  membersList: CharacterItem[];
  onCloseModal: () => void;
}) {
  ...
  const { performShortRest, isPending } = usePerformShortRest(
    Number(campaignId),
    handleSuccess
  );
  ...
  return (
    <Container>
      <Heading as="h2">Short rest</Heading>
      <Box style={{ gridRow: "2/5" }}>
        <p style={{ marginBottom: "10px" }}>Select Characters:</p>
        {members.map((e) => (
          <MemberSelect
