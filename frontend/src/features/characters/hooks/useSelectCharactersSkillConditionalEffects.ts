...
export function useSelectCharactersSkillConditionalEffects(onSuccess: () => void, characterId: number, skill: skill) {
  ...
  const { mutate: selectSkillRollConditionalEffects, isPending } = useMutation({
    ...
    }) => selectSkillRollConditionalEffectsApi(characterId, skill, conditionalEffects.filter(e => e.selected).map(e => e.effectId)),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["characterSkillConditionalEffects", characterId, skill] });
      onSuccess();
    },
    onError: (error) => {
      console.error(error);
