...
import { updateCoinSack as updateCoinSackApi } from "../../../services/apiCharacters";
import { CoinPurse } from "../../items/models/coinPurse";

export function useUpdateCharacterCoinSack(
  characterId: number,
  onSuccess: () => void
) {
  ...
  const {
    ...
    mutationFn: ({ coinSack }: { coinSack: CoinPurse }) =>
